...
export const getFromStore = (obj) => async (dispatch) => {
  try {
    ...
    dispatch({
      type: actionTypes.get_all_from_store,
      payload: data.allStors.stores,
    });
  } catch (error) {
...
export const updateStores = (id, obj) => async (dispatch) => {
  try {
    const { data } = await api.putData("/store/update/" + id, obj);
    dispatch({
      type: actionTypes.update_from_store,
      payload: data.updateStore,
    });
  } catch (error) {
    console.log(error.message);
  }
